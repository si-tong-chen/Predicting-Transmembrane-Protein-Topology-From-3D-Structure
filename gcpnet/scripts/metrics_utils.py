...
def test_experiment_and_write_results_to_file(cv, experiment_file_path, test_loader):
    ...
    tm_type_acc = float(
        calculate_acc(confusion_matrix[0][0] + confusion_matrix[0][5], confusion_matrix[0].sum()).detach()
    )
    tm_sp_type_acc = float(
        calculate_acc(confusion_matrix[1][1] + confusion_matrix[1][5], confusion_matrix[1].sum()).detach()
    )
    sp_type_acc = float(
        calculate_acc(confusion_matrix[2][2] + confusion_matrix[2][5], confusion_matrix[2].sum()).detach()
    )
    glob_type_acc = float(
        calculate_acc(confusion_matrix[3][3] + confusion_matrix[3][5], confusion_matrix[3].sum()).detach()
    )
    beta_type_acc = float(
        calculate_acc(confusion_matrix[4][4] + confusion_matrix[4][5], confusion_matrix[4].sum()).detach()
    )

    experiment_json["test"][cv]["confusion_matrix"] = confusion_matrix.tolist()
    experiment_json["test"][cv].update({"type": type_accuracy})

    # Topology
    experiment_json["test"][cv].update({"tm": {"type": tm_type_acc, "topology": tm_accuracy}})

    experiment_json["test"][cv].update({"sptm": {"type": tm_sp_type_acc, "topology": sptm_accuracy}})

    experiment_json["test"][cv].update({"sp": {"type": sp_type_acc, "topology": sp_accuracy}})

    experiment_json["test"][cv].update({"glob": {"type": glob_type_acc, "topology": glob_accuracy}})

    experiment_json["test"][cv].update({"beta": {"type": beta_type_acc, "topology": beta_accuracy}})
